langS_En = {
	cdr: {
		...
		field_filter_help: {
			...
			sipcallerdip_encaps: 'Searching for more ip encapsulation addresses (OR): 192.168.0.1 10.0.0.0/16, searching for subnet except one IP: 10.0.0.0/16 !10.0.0.1, searching for more subnets except another subnet: 10.0.0.0/16 192.168.0.0/16 !10.0.20.0/24 !192.168.10.0/24. If not proper IP or Net then DNS resolve is tried.'
		},
		button: {
		...
		menu: {
			...
			from_own_clients: 'From own clients',
			from_own_servers: 'From own servers',
			from_public_trunks: 'From public trunks',
			by_source_ip: 'by source IP',
			by_source_number: 'by source Tel.number'
		},
		tbar: {
		...
		title: {
			...
			reportProblemsTab: 'Report problems',
			noteTab: 'note',
			pieChart: 'pie chart',
			...
			report_problems_from_own_clients: 'Report problems - From own clients',
			report_problems_from_own_servers: 'Report problems - From own servers',
			report_problems_from_public_trunks: 'Report problems - From public trunks'
		},
		text: {
	...
	group_stat: {
		menu: {
			...
			sip_src_ip_encaps: 'SIP source IP encapsulation',
			sip_dst_ip_encaps: 'SIP destination IP encapsulation',
			sip_dst_ip: 'SIP destination IP',
	...
	register: {
		...
		field_filter: {
			...
			counter: 'Counter',
			ip_address: 'IP address',
			ip_address_encaps: 'IP encapsulation',
			caller_ip: 'Caller IP',
			caller_ip_encaps: 'Caller IP encapsulation',
			called_ip: 'Called IP',
	...
	dashboard_d: {
		field: {
			...
			group_by: 'group by',
			hide_rows_without_problems: 'hide rows without problems'
		},
		field_help: {
		...
		combomenu: {
			...
			typePanel_asr_acd_mos_by_sip_ip_encaps_grid: 'Custom CDR grid - by SIP IP encapsulation',
			...
			typePanel_report_problems: 'Report problems',
	...
	alerts_daily_reports: {
		...
		combomenu: {
			...
			summary: 'summary',
			cc_aggreg_type_max: 'max',
			cc_aggreg_type_avg: 'avg',
	...
	report_problems: {
		column: {
			...
			source_number: 'Source Number',
			cnt_all: '# All',
			cnt_connected: '# Connected',
			cnt_mos_lt: '# MOS < ',
			cnt_interrupted_calls: '# Interrupted',
			cnt_one_way: '# One way',
			cnt_missing_rtp: '# Missing RTP',
			cnt_missing_srtp_key: '# Missing SRTP key',
			cnt_fas: '# FAS',
			cnt_zerossrc: '# Zero SSRC',
			cnt_sipalg: '# SIP ALG'
		},
		field: {
			groupBy: 'group by',
			...
			interval: 'interval',
			hide_rows_without_problems: 'hide rows without problems',
			mos_intervals: 'MOS intervals',
			percent_intervals: 'percent intervals',
			exception: 'exception',
			interval_limit: 'limit',
			mos_interval_limit: 'MOS <',
			percent_interval_limit: 'percent >',
			color: 'color',
			type: 'type',
			interval: 'interval',
			MOS: 'MOS',
			filters: 'filters',
			sip_src_ip: 'SIP IP src',
			sip_dst_ip: 'SIP IP dst',
			rtp_src_ip: 'RTP IP src',
			rtp_dst_ip: 'RTP IP dst',
			src_phone_num: 'number src',
			dst_phone_num: 'number dst'
		},
		combomenu: {
			source_ip: 'source IP',
			source_number: 'source Tel.number',
			// client_id: 'client ID',
			mos_compare: 'MOS compare',
			interrupted_calls: 'interrupted calls',
			one_way: 'one way',
			missing_rtp: 'missing RTP',
			missing_srtp_key: 'missing SRTP key',
			fas: 'FAS',
			zerossrc: 'zero SSRC',
			sipalg: 'SIP ALG',
		...
		button: {
			...
			allProblemsBy_source_ip: 'all problems by source IP',
			allProblemsBy_source_number: 'all problems by source Tel.number'
		},
		grid: {
			subject_mos_interval: 'interval',
			subject_percent_interval: 'interval'
		},
		text: {
			missingTrunksInIPGroups: 'no \'trunks\' defined in groups of IP address',
			missingServersInIPGroups: 'no \'servers\' defined in groups of IP address',
			question_doYouWantDefinitionIPGroups: 'Do you want to enter the groups of IP address definition?',
			message_missingNecessaryRecords: 'Existence of these records is necessary to enter the requested operation.',
			waitForRender: 'wait for render',
			waitForLoading: 'wait for loading',
			loading: 'loading',
			noProblemFound: 'no problem found',
			missingServersDefinition: 'missing servers definition',
			missingTrunksDefinition: 'missing trunks definition'
		},
		title: {
			report_problems: 'Report problems',
			report_problems_from_own_clients: 'Report problems - From own clients',
			report_problems_from_own_servers: 'Report problems - From own servers',
			report_problems_from_public_trunks: 'Report problems - From public trunks',
			problems_by_source_ip: 'Problems by source IP',
			problems_by_source_number: 'Problems by source Tel.number',
			settings: 'Settings',
			mos_compare: 'MOS compare',
			exceptions: 'Exceptions',
			relatedCDR: 'CDR',
			chartsCDR_call_quality: 'charts - call quality',
			chartsCDR: 'charts',
			MOS: 'MOS',
			loss: 'loss',
			jitter: 'jitter',
	...
	capture_rules: {
		...
		field_help: {
			...
			options: 'Record OPTIONS messages. The processing engine needs to be enabled. See sip-options parameter in the voipmonitor.conf.',
			notify: 'Record NOTIFY messages. The processing engine needs to be enabled. See sip-notify parameter in the voipmonitor.conf.',
			subscribe: 'Record SUBSCRIBE messages. The processing engine needs to be enabled. See sip-subscribe parameter in the voipmonitor.conf.',
			dtmf: 'Record DTMF ?',
	...
	global: {
		...
		updateGeoIP_confirmQuery: 'Do you want to update the GeoIP country table now?'
	},
	arrays: {
		...
		LauditActivity_getVoiceRecording: 'API getVoiceRecording',
		LauditActivity_getPCAP: 'API getPCAP',
		LauditActivity_handleActiveCall: 'API handleActiveCall',
		LauditActivity_getVoipCalls: 'API getVoipCalls',
		LauditActivity_getShareURL: 'API getShareURL',
		...
		LtimeInterval_specified: 'specified',
		...
		LtypeChart_TCH_delay_avg_intervals: 'x̄ PDV - intervals',
